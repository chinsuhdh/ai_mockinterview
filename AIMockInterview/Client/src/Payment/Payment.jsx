...
export default function Dashboard() {
    ...
    const [userRole, setUserRole] = useState('user'); // Sẽ chứa: user, basic, premium, ultra...
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                ...
                // 1. KIỂM TRA REDIRECT TỪ PAYOS
                const searchParams = new URLSearchParams(location.search);
                const isFromPayment = searchParams.get('cancel') === 'false' || searchParams.get('status') === 'PAID';

                if (isFromPayment) {
                    setPaymentSuccessMsg(true);
                    // Dọn dẹp URL cho đẹp, tránh F5 bị load lại param
                    window.history.replaceState({}, document.title, "/dashboard");
                    // Delay 2 giây để chờ Webhook của Backend xử lý xong Database
                ...
                // Lấy thông tin cá nhân và gói cước
                const profileData = profileRes.data?.data || profileRes.data;
                if (profileData) {
                    setUserName(profileData.fullName || profileData.FullName || 'Bạn');
                    setUserRole((profileData.role || profileData.Role || profileData.planName || 'user').toLowerCase());
                }
                ...
                // Lấy dữ liệu Radar Chart (Lỗ hổng kỹ năng)
                ...
                setLoading(false);
                // Ẩn thông báo thanh toán thành công sau 5 giây
        ...
        return (
            <div className="min-h-screen bg-[#F4F4F5] flex items-center justify-center font-sans">
                <div className="flex flex-col items-center gap-4 text-neutral-500">
                    ...
                    {paymentSuccessMsg && <p className="text-sm text-green-600 animate-pulse">Đang xác nhận thanh toán của bạn...</p>}
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-[#F4F4F5] p-4 sm:p-6 md:p-8 font-sans text-neutral-900 selection:bg-amber-100">
            ...
            <div className="max-w-[1400px] mx-auto space-y-6">
                ...
                {paymentSuccessMsg && !error && (
                    <div className="flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-2xl text-green-700 font-semibold animate-fade-in-down">
                        <CheckCircle2 size={20} className="shrink-0" />
                        <span>Thanh toán thành công! Gói cước của bạn đã được cập nhật.</span>
                    </div>
                )}
                ...
                <div className="relative overflow-hidden bg-neutral-900 rounded-3xl p-8 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 border border-neutral-800 animate-fade-in-up">
                    ...
                    <div className="relative z-10 text-center md:text-left">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-amber-300 font-bold text-xs uppercase mb-3 border border-white/10 backdrop-blur-md">
                            <Sparkles size={14} /> Gói hiện tại: {userRole.toUpperCase()}
                        </div>
                        ...
                        <div className="bg-white p-6 rounded-3xl shadow-sm border border-neutral-100 h-full flex flex-col">
                            ...
                            <div className="space-y-3 flex-1 max-h-[350px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-neutral-200">
                                {sessions.length > 0 ? sessions.map((session, idx) => {
                                    ...
                                    return (
                                        <div key={id ?? idx} onClick={() => navigate(`/dashboard/session/${id}`)} className="p-4 rounded-2xl border border-neutral-100 hover:border-amber-200 hover:shadow-md transition-all cursor-pointer bg-neutral-50 hover:bg-white group">
                            ...
                            {/* --- QUẢN LÝ GÓI CƯỚC FIX LỖI BASIC --- */}
                                ...
                                {['premium', 'pro', 'ultra'].includes(userRole) ? (
                                    <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl p-4 text-center">
                                        <div className="flex items-center justify-center gap-1.5 font-black text-amber-700 text-sm uppercase tracking-wide">
                                            <Crown size={16} className="fill-amber-500 text-amber-600" /> Tài khoản {userRole}
                                        </div>
                                    ...
                                    <>
                                        ...
                                        <button onClick={() => navigate('/#pricing')}
                                            ...
                                            <Crown size={14} className="text-amber-400" /> Nâng cấp Basic/Pro
                                        </button>
                                    </>
                                )}
                            </div>
